...
class RDOChallengesBot:
    ...
    def translate_challenge(self, challenge):
        ...
        if title_code in self.challenge_translations:
            ...
            arabic_title = f"تحدي: {title_code.replace('MPGC_', '').replace('MPRC_', '').replace('_', ' ')}"
...
def main():
    ...
    if not webhook_url:
        print("❌ خطأ: DISCORD_WEBHOOK_URL غير موجود في المتغيرات")
        print("تأكد من إضافة Discord webhook URL في GitHub Secrets")
        return False

    # إنشاء البوت وإرسال التحديات
